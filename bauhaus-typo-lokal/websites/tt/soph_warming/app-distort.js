(() => {
  ...
  // Text scale (keep at 1 for consistency)
  const TEXT_SCALE = 1;
  ...
  function linesToMaskSmall(lines) {
    ...
    let fontSize = Math.floor(PH * 0.6 * TEXT_SCALE);
    ...
    while (!fits && fontSize > 10) {
      ...
      if (maxW <= PW - margin * 2 && totalH <= PH - margin * 2) fits = true;
      else fontSize -= 3;
    }
  ...
  function renderMaskToDisplay(mask) {
    ...
    const pad = 12;
    const baseScale = Math.min((DW - 2 * pad) / PW, (DH - 2 * pad) / PH);
    const scale = baseScale * CONTENT_SCALE;
    const offsetX = DW / 2 - (PW / 2) * scale;
    const offsetY = DH / 2 - (PH / 2) * scale;
