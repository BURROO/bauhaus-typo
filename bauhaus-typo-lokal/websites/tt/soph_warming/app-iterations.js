(() => {
  ...
  function renderBands(mask,w,h,level,targetCtx,targetCanvas){
    ...
    paintMaskToRGBA(baseMask,w,h,out,[255,255,255]);
    ...
    for(let i=1;i<=rings;i++){
      ...
      const outer = dilate(baseMask,w,h,rOuter);
  ...
  function scheduleFastRender(){
    ...
    rafPending=true;
    requestAnimationFrame(()=>{ rafPending=false; process(lastMaskFast,lastMaskHQ); });
  }
  ...
  function onWheelCanvas(e){
    e.preventDefault();
    if(!hasRenderedOnce){ hasRenderedOnce=true; scheduleFastRender(); return; }
    outlineLevel=clamp(outlineLevel+e.deltaY*0.05,0,100);
    scheduleFastRender();
  }
  ...
  function init(){
    ...
    loadPrevImage().then(img=>{
      ...
      if(lastMaskFast) drawBaseOnly(lastMaskFast);
      diag(`Iterations ready (base shown, rings on scroll; center-fit, dual-pass: fast ${PW_FAST}x${PH_FAST}, HQ ${PW_HQ}x${PH_HQ}).`);
    });
    ...
    if(nextLink){
      nextLink.addEventListener('click',(e)=>{
        e.preventDefault();
        saveCurrent();
        const target = nextLink.getAttribute('href') || 'index.html';
