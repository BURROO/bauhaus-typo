...
function syncOverlayTextarea() {
	...
	if (state.fontFamily) {
		state.el.overlayTextarea.style.fontFamily = `'${state.fontFamily}', sans-serif`;
	}
...
function initTextEdit() {
	...
	document.addEventListener("mousedown", (e) => {
		if (
			state.textEditMode &&
			state.el.overlayTextarea &&
			!state.el.overlayTextarea.contains(e.target)
		) {
			exitTextEditMode();
		}
	});
	...
	document.addEventListener("keydown", (e) => {
		if (e.key === "t" || e.key === "T") {
			const targetIsOverlay =
				state.el.overlayTextarea &&
				document.activeElement === state.el.overlayTextarea;
