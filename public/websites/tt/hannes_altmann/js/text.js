...
function clearLettersOnly() {
	...
	for (let child of paper.project.activeLayer.children.slice()) {
		if (
			child === state.textField ||
			child === state.handle ||
			child === state.mouseRadiusCircle ||
			child === state.mouseRadiusSquare
		)
			continue;
...
function renderText() {
	...
	state.fontSize =
		parseFloat(state.el?.sliderFont?.value || state.fontSize) || state.fontSize;
