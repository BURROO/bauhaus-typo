...
function setupFontDrop() {
	...
	["dragenter", "dragover"].forEach((evt) =>
		target.addEventListener(evt, noop),
	);
	...
	target.addEventListener("drop", async (e) => {
		...
		try {
			...
			if (state.el?.overlayTextarea) {
				state.el.overlayTextarea.style.fontFamily = `'${fontName}', sans-serif`;
			}
			renderText();
		} catch (err) {
...
function initInteractions() {
	...
	paper.view.onMouseDown = function (event) {
		...
		if (hit) {
			...
			if (
				hit.item === state.textField ||
				state.textField.contains(event.point)
			) {
				state.draggingTextField = true;
				state.pointerGrabOffset = state.textField.position.subtract(
					event.point,
				);
	...
	paper.view.onMouseUp = function (event) {
		if (state.draggingTextField) {
			...
			const dragTranslation = state.textField.position.subtract(
				state.initialTextFieldPosition,
			);
	...
	paper.view.onMouseMove = function (event) {
		...
		requestAnimationFrame(() => {
			...
			if (state.lastMouseModifiers && state.lastMouseModifiers.shift) {
				...
				const delta = state.lastMousePoint.subtract(
					state.shiftConstrainedShapeStart,
				);
				// Determine dominant axis: if |dx| > |dy|, lock to horizontal; else lock to vertical
				if (Math.abs(delta.x) > Math.abs(delta.y)) {
					// Horizontal: keep start Y, use current X
					shapePos = new paper.Point(
						state.lastMousePoint.x,
						state.shiftConstrainedShapeStart.y,
					);
				} else {
					// Vertical: keep start X, use current Y
					shapePos = new paper.Point(
						state.shiftConstrainedShapeStart.x,
						state.lastMousePoint.y,
					);
				}
			} else {
			...
			// While panel is animating, keep visuals responsive but skip force calculations
			if (state.panelHovered) {
				return;
			}

			state.forceStrength =
				parseFloat(state.el.sliderForce?.value || state.forceStrength) ||
				state.forceStrength;

			// Apply the selected force type based on shape mode
			if (state.shapeMode === "square") {
				// Square wall-like forces
				if (state.forceType === "push") {
					applySquarePushForce(
						state.letters,
						shapePos,
						state.mouseRadius,
						state.forceStrength,
						state.mouseRadiusSquare.forceDirection,
					);
				} else if (state.forceType === "split") {
					applySquareSplitForce(
						state.letters,
						shapePos,
						state.mouseRadius,
						state.forceStrength,
				...
				} else if (state.forceType === "spiral") {
					applySquareSpiralForce(
						state.letters,
						shapePos,
						state.mouseRadius,
						state.forceStrength,
				...
				} else if (state.forceType === "jitter") {
					applySquareJitterForce(
						state.letters,
						shapePos,
						state.mouseRadius,
						state.forceStrength,
				...
				} else if (state.forceType === "bell") {
					applySquareBellForce(
						state.letters,
						shapePos,
						state.mouseRadius,
						state.forceStrength,
